...
WEIGHT_CANDIDATES = [
    os.path.join(_HERE, 'models', 'best_model.npy'),
    os.path.join(_HERE, 'models', 'last_model.npy'),
    os.path.join(_HERE, 'models', 'nafnet_gan_finetuned_best.pth'),
    os.path.join(_HERE, 'models', 'nafnet_gan_generator_best.pth'),
    os.path.join(_HERE, 'outputs', 'best_model.npy'),
    os.path.join(_HERE, 'outputs', 'last_model.npy'),
]
...
def load_model(device):
    ...
    if weight_path.endswith('.npy'):
        state = np.load(weight_path, allow_pickle=True).item()
    else:
...
def resolve_input_dir(input_dir):
    """Accept both a folder containing .npy files directly and a parent folder with a NoisyLR subfolder."""
    candidates = [
        os.path.abspath(input_dir),
        os.path.join(os.path.abspath(input_dir), 'NoisyLR'),
        os.path.join(os.path.abspath(input_dir), 'noisylr'),
    ]

    for cand in candidates:
        files = sorted(glob.glob(os.path.join(cand, '*.npy')))
        if files:
            return cand, files

    return os.path.abspath(input_dir), sorted(glob.glob(os.path.join(os.path.abspath(input_dir), '*.npy')))
...
def main():
    ...
    resolved_input_dir, files = resolve_input_dir(input_dir)
    if not files:
        print(f"No .npy files found in {input_dir} or in {os.path.join(input_dir, 'NoisyLR')}")
        sys.exit(1)

    print(f"Using input directory: {resolved_input_dir}")
